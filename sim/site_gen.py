...
<div class="stacks">
  <div class="card ok"><h3>gym-pybullet-drones</h3><span class="st">&#10003; VALIDATED NATIVELY</span><p>24 kHz control loop, custom paper-vehicle URDF. Step agrees with MuJoCo to 0.3% (17.63&deg; vs 17.71&deg;).</p></div>
  <div class="card ok"><h3>MuJoCo</h3><span class="st">&#10003; VALIDATED NATIVELY</span><p>20 kHz physics, contact floor. All three paper scenarios; flip settles 2.93 s, fully airborne (3 seeds).</p></div>
...
<p class="note"><b>With identical gains, references, and the paper's &plusmn;0.1 noise model, sine tracking agrees across all four stacks — and the real-firmware stack is the best of them:</b> ArduPilot 14.7&deg; &plusmn; 0.2, Gazebo 17.3&deg; &plusmn; 0.0, gym-pybullet 17.52&deg;, MuJoCo 17.61&deg;. The two native engines agree on step to 0.3% (17.71&deg; vs 17.63&deg;). The 360&deg; flip completes on real firmware (&phi; settles 2.89 s) and MuJoCo (2.93 s) — near-identical — while PyBullet/Gazebo sit in the engine-dependent knife-edge regime, analyzed as the P&sup2; pursuit equilibrium (Eq. 33 in the repo's file map).</p>
...
<ol class="findings">
<li><b>The controller transfers across stacks.</b> Sine agrees within 3&deg; across all four (firmware best); native step agrees to 0.3%.</li>
...
<p style="font-size:14px">Every number and figure on this page is machine-generated by <code>sim/run_all.sh</code> + <code>python -m sim.compare</code> + <code>python -m sim.site_gen</code> — nothing hand-edited. Full source, tests (41), and derivations: <a href="{BRANCH_URL}">sainath/sim-deployment branch</a>.</p>

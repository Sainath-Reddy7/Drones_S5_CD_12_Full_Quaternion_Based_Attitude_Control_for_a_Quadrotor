...
(nohup timeout 400 ./build/sitl/bin/arducopter --model JSON --speedup 1 \
    --serial0=tcp:5760 \
    --defaults=/root/ardupilot/Tools/autotest/default_params/copter.parm,/root/ardupilot/Tools/autotest/default_params/gazebo-iris.parm,/root/bridge.parm \
...
mkdir -p /root/inbox_gz
latest=$(ls -t "$REPO"/results/*.csv 2>/dev/null | head -1)
if [ -n "$latest" ]; then
  tag="${SCEN}"
  [ "${STEP_AMP:-1.0}" = "0.7" ] && tag="step070"
  cp "$latest" "/root/inbox_gz/tag_${tag}_seed${SEED}.csv"
  echo "[gz-run] filed tag_${tag}_seed${SEED}.csv"
fi

...
Two self-contained HTML files (no server, no dependencies — just open in a browser):

| File | Description |
|------|-------------|
| **`quadrotor_live.html`** | **Live interactive simulator** — the full physics (eq. 17–21) runs in JavaScript in your browser. Fly the drone with reference sliders, tune gains ($P_q$, $P_\omega$), noise, and torque limits in real-time, or run preset scenarios (step / sine / flip). Drag to orbit the 3D camera. |
| **`quadrotor_sim.html`** | **Playback viewer** — replays pre-computed SITL trajectories as a 3D animation with tracking plots. Generated by `build_ui.py`. |

Both render a wireframe quadrotor (arms, spinning rotors, RGB body axes with X/Y/Z labels) plus live attitude-tracking and torque plots matching the paper's Figures 3–8.

The live simulator's JavaScript physics is verified numerically identical to the Python SITL (max difference $4.4 \times 10^{-11}$ per RK4 step).

## Equation reference

See **`equations.md`** for every equation (1)–(21) from the paper in LaTeX, with code-location mapping, parameter table, sign-convention analysis, and verification summary.

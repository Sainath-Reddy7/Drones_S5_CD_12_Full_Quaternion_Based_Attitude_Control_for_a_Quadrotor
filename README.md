...
| **ArduPilot SITL** (real firmware, compiled from source) | ✅ **flown under WSL2** | GUIDED → arm → takeoff → 50 Hz setpoints; all 3 scenarios; position hold keeps drift at **0.03 m** |
| **Gazebo** (gz-harmonic 8.15 + ardupilot_gazebo) | ✅ **flown under WSL2** | full physics chain; all 3 scenarios; **sine 17.28° RMS — within 0.4° of the native stacks** |

## Headline

With identical gains, references, and the paper's ±0.1 noise model, **sine
tracking agrees across every physics stack that ran it**: MuJoCo 17.61°,
gym-pybullet-drones 17.52°, Gazebo (real firmware) 17.28° — a 0.33° spread
across three independent engines; step agrees 17.71°/17.68° between the two
native stacks. On the production flight stack (ArduPilot) the same law runs
as an outer loop and the added stack lag shows exactly where FRP §7 predicted
it (sine 37.7°). The 360° flip completes deterministically on MuJoCo and
ArduPilot (2.3 s settle) and hits engine-dependent limits elsewhere —
analyzed as the P² pursuit equilibrium ω = 5·sin(e/2) (Eq. 33).
